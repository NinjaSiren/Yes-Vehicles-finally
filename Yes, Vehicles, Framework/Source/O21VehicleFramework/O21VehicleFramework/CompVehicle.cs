...
namespace O21VehicleFramework
{
    ...
    public class CompVehicle : ThingComp
    {
        ...
        public bool CanManipulate => Props.manipulationHandling != HandlingType.Incapable && ManipulationHandlerAvailable;

        //Toggles for whether or not a vehicle can be refueled or repaired.
        public bool refuelToggle = true;
        public bool repairToggle = true;

        //Whether or not the vehicle is in a position to be claimed by the player.
        ...
        public void ResolveFactionPilots()
        {
            ...
            // N17 - Improvement could be made to check pawnKind for preferred handlers through defModExt instead.
            // Logic: Different factions could be using these vehicles through different pawnKinds, it'd be silly
            //        to create a new 'race' entirely every time they needed to change.
        ...
        public void ResolveFactionFuel()
        {
            if (Pawn.Faction != null && !Pawn.Faction.IsPlayer)
            {
                if (!ResolvedFuel)
                {
                    ...
                    if(comp != null)
                    {
                        comp.Refuel(comp.TargetFuelLevel - comp.Fuel);
                    }
                }
            }
        }
        ...
        public void ResolveEjection()
        {
            ...
            if (parent is Pawn vehicle && vehicle.Spawned && handlers != null && handlers.Count > 0 && !(Pawn.GetLord()?.LordJob?.ToString() == "RimWorld.LordJob_FormAndSendCaravan") && !(Pawn.CurJob?.def == JobDefOf.UnloadYourInventory) && !Pawn.Position.InNoBuildEdgeArea(Pawn.Map) && !Pawn.IsFighting())
            {
                //Do not eject anyone from the vehicle for needs during combat.
                //It's too dangerous to leave during combat.
                if (this?.Pawn?.Map?.attackTargetsCache?.GetPotentialTargetsFor(Pawn)?.FirstOrDefault(x => !x.ThreatDisabled(vehicle)) == null)
                {
                    //TODO May require further investigation for proper update
                    foreach (var group in handlers)
                    {
                        var toEject = group?.handlers?.InnerListForReading?.FirstOrDefault(x => !x.Spawned && x?.needs?.AllNeeds?.FirstOrDefault(y => y.CurLevelPercentage < Props.ejectIfBelowNeedPercent) != null);
                        if (toEject != null)
                        {
                            Messages.Message("MessagePawnLeftVehicle".Translate(toEject.Label, Pawn.Label, "low"), Pawn, MessageTypeDefOf.NegativeHealthEvent);
                            Eject(toEject);
                        }
                    }
                }
        ...
        public void ResolveStatus()
        {
            ...
            movingStatus = GetMovingState(compRefuelable);
            weaponStatus = GetWeaponState(compRefuelable);
            manipulationStatus = GetManipulationState();

            if (compRefuelable != null)
            {
                //If it isn't moving than it shouldn't use fuel
                if (Pawn.pather != null && !Pawn.pather.Moving || Pawn.GetCaravan() != null && Pawn.GetCaravan().CantMove)
                {
            ...
            if (weaponStatus != WeaponState.able && (compRefuelable != null && !compRefuelable.HasFuel || compRefuelable == null))
            {
                if (this?.Pawn?.CurJob?.def == JobDefOf.Wait_Combat || this?.Pawn?.CurJob?.def == JobDefOf.AttackStatic || this?.Pawn?.CurJob?.def == JobDefOf.AttackMelee)
                {
            ...
            if (movingStatus == MovingState.frozen)
            {
            ...
            if (movingStatus != MovingState.able && weaponStatus != WeaponState.able && this?.Pawn?.mindState != null)
            {
            ...
            {
                Messages.Message("Cannot draft immobile vehicle", MessageTypeDefOf.CautionInput);
                Pawn.drafter.Drafted = false;
            }
        }

        public MovingState GetMovingState(CompRefuelable fuel)
        {
            if (!MovementHandlerAvailable && movingStatus == MovingState.able && Props.movementHandling != HandlingType.NoHandlerRequired)
            {
                return MovingState.frozen;
            }
            if(fuel != null)
            {
                if (!fuel.HasFuel)
            ...
            return MovingState.able;
        }

        public WeaponState GetWeaponState(CompRefuelable fuel)
        {
            if (!WeaponHandlerAvailable && weaponStatus == WeaponState.able && Props.weaponHandling != HandlingType.NoHandlerRequired)
            {
                return WeaponState.frozen;
            }

            return WeaponState.able;
        }

        public ManipulationState GetManipulationState()
        {
            if (!ManipulationHandlerAvailable && manipulationStatus == ManipulationState.able && Props.manipulationHandling != HandlingType.NoHandlerRequired)
            {
                return ManipulationState.frozen;
            }

            return ManipulationState.able;
        }
        ...
        public void ResolveFaction()
        {
            if(this.parent.Faction != Faction.OfPlayer)
            {
                if(ResolvedPawns && AllOccupants.Count <= 0 && !isClaimable)
                {
                    isClaimable = true;
        ...
        public override IEnumerable<Gizmo> CompGetGizmosExtra()
        {
            ...
            if (Pawn.Faction != null && Pawn.Faction == Faction.OfPlayer)
            {
                ...
                // Refuel Toggle
                if (this.parent.def.HasComp(typeof(CompRefuelable)))
                {
                    CompRefuelable comp = this.parent.TryGetComp<CompRefuelable>();
                    Command_Toggle com = new Command_Toggle();
                    com.icon = TexCommandVehicle.Refuel;
                    com.isActive = (() => this.refuelToggle);
                    com.defaultLabel = "VehicleRefuelToggleLabel".Translate();
                    if (this.refuelToggle)
                    {
                        com.defaultDesc = "VehicleRefuelToggleDesc".Translate();
                    }
                    else
                    {
                        com.defaultDesc = "VehicleRefuelToggleDesc".Translate();
                    ...
                    {
                        this.refuelToggle = !this.refuelToggle;
                        if (refuelToggle)
                        {
                            comp.TargetFuelLevel = comp.Props.fuelCapacity;
                        }
                        else
                        {
                            comp.TargetFuelLevel = 0f;
                        }
                    };
                ...
                {
                    Command_Toggle com = new Command_Toggle();
                    com.icon = TexCommandVehicle.Repair;
                    com.isActive = (() => this.repairToggle);
                    com.defaultLabel = "VehicleRepairToggleLabel".Translate();
                    if (this.repairToggle)
                    {
                        com.defaultDesc = "VehicleRepairToggleDesc".Translate();
                    }
                    else
                    {
                        com.defaultDesc = "VehicleRepairToggleDesc".Translate();
                    ...
                    com.toggleAction = delegate ()
                    {
                        this.repairToggle = !this.repairToggle;
                    };
        ...
        {
            base.PostExposeData();
            Scribe_Values.Look(ref ResolvedFuel, "ResolvedFuel", false);
            ...
            Scribe_Values.Look(ref refuelToggle, "refuelToggle", true);
            Scribe_Values.Look(ref repairToggle, "repairToggle", true);
            Scribe_Values.Look(ref isClaimable, "isClaimable", false);

            Scribe_Collections.Look(ref vehicleContents, "vehicleContents", LookMode.Deep); //Stores the handlergroups of the vehicle and its pawns while the vehicle is in a caravan
