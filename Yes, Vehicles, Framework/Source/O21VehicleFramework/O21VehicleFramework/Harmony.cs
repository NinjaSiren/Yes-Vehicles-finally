...
namespace O21VehicleFramework
{
    ...
    internal static class Harmony
    {
        static Harmony()
        {
            var harmony = HarmonyInstance.Create("rimworld.outpost21.vehicleframework");
            ...
            harmony.Patch(
                ...
                new HarmonyMethod(typeof(Harmony),
            ...
            harmony.Patch(
                ...
                null, new HarmonyMethod(typeof(Harmony),
            ...
            harmony.Patch(AccessTools.Method(typeof(FloatMenuMakerMap), "AddUndraftedOrders"), null, null, new HarmonyMethod(typeof(Harmony),
        ...
        //{
        //    static bool Prefix(IntVec3 start, LocalTargetInfo dest, TraverseParms traverseparms, PathFinder __instance, ref PawnPath __result, PathEndMode pemode = PathEndMode.OnCell)
        //    {
        ...
        {
            // NO21 - Longer code but at least it works.
            if (pawn.GetComp<CompVehicle>() != null)
            {
                CompVehicle comp = pawn.GetComp<CompVehicle>();
            ...
            //if (pawn.GetComp<CompVehicle>() != null && (pawn.GetComp<CompVehicle>().movingStatus == MovingState.frozen || !pawn.GetComp<CompVehicle>().CanManipulate || pawn.Drafted))
            //{
            //    __result = null;
            //    return false;
            //}
        ...
        public static void DontRescueVehiclesInFloatMenus(Vector3 clickPos, Pawn pawn, List<FloatMenuOption> opts)
        {
            ...
            if (groundPawn != null)
                ...
                {
                    //Remove "Rescue" option from right click.
        ...
        {
            if (__result != null && pawn?.GetComp<CompVehicle>() is CompVehicle compVehicle &&
...
#pragma warning restore IDE0019 // Use pattern matching
        ...
        public static bool CheckForErrors_PreFix(Dialog_FormCaravan __instance, List<Pawn> pawns, ref bool __result)
        {
            if (pawns.FindAll(x => x.GetComp<CompVehicle>() != null) is List<Pawn> vehicles)
            {
                ...
                var pawn = pawns.Find(x => (x.TryGetComp<CompVehicle>() == null || x.TryGetComp<CompVehicle>() is CompVehicle v && !v.CanMove) && !pawns.Any(y => !y.IsPrisoner && !y.RaceProps.Animal && y.CanReach(x, PathEndMode.Touch, Danger.Deadly, false, TraverseMode.ByPawn)));
                if (pawn != null)
                {
                        ...
                        if (countToTransfer > 0)
                        {
                            for (var j = 0; j < __instance.transferables[i].things.Count; j++)
                            {
                                ...
                                if (!t.Spawned || pawns.Any(x =>
                                        (x.IsColonist || x.TryGetComp<CompVehicle>() is CompVehicle v && v.CanMove) && x.CanReach(t, PathEndMode.Touch, Danger.Deadly, false, TraverseMode.ByPawn)))
                                {
                                    num += t.stackCount;
        ...
        */
        ...
        #region OutpostTweaksMethods
        // Adjustments, Fixes and Additions written by Neronix17

        // Allows DefModExt_RenderWeaponExt to show weapon at all times on vehicles.
        ...
        // Forces the weapon to point the same direction as the vehicle.
        // Probably shouldn't be doing it this way but I'm not good enough to think of a better way.
        [HarmonyPatch(typeof(PawnRenderer), "DrawEquipmentAiming")]
        public static class PawnRenderer_DrawEquipmentAiming_Patch
        {
            [HarmonyPriority(Priority.First)]
            static void Prefix(PawnRenderer __instance, ref float aimAngle)
            {
        ...
        // Allows DefModExt_RenderWeaponExt to offset the location of a the weapon on a vehicle.
        // Probably shouldn't be doing it this way but I'm not good enough to think of a better way.
        ...
        // Sets Vehicles to always be counted as "InBed", so they can have surgery performed at any time.
        [HarmonyPatch(typeof(RestUtility), "InBed")]
        public static class RestUtility_InBed_Patch
        {
            static bool Prefix(Pawn p, bool __result)
            {
                if (p.def.HasComp(typeof(CompVehicle)))
                {
                    __result = true;
                    return false;
                }
        ...
        public static class TransferableUtility_CanStack_Patch
        {
            static bool Prefix(Thing thing, ref bool __result)
            {
                if (thing.def.category == ThingCategory.Pawn)
                {
                    ...
                    if (pawn.def.HasComp(typeof(CompVehicle)))
                    {
                        __result = false;
                        return false;
                    }
        ...
        public static class IsColonistPatch
        {
            ...
            {
                if (__instance.def.HasComp(typeof(CompVehicle)))
                {
        ...
        public static class Caravan_IsPlayerControlled
        {
            static void Postfix(Caravan __instance, ref bool __result)
            {
                if (!__result)
                {
                    __result = __instance.PawnsListForReading.Any(p => p.Faction == Faction.OfPlayer && (p.TryGetComp<CompVehicle>()?.AllOccupants.Any() ?? false));
                }
            }
        }
        ...
        public static class MapPawns_AnyPawnblockingMapRemoval_Patch
        {
            static void PostFix(MapPawns __instance, ref bool __result)
            {
                if (!__result)
                {
                    __result = __instance.AllPawns.ToList().Any(p => p.Faction == Faction.OfPlayer && (p.TryGetComp<CompVehicle>()?.AllOccupants.Any() ?? false));
                }
            }
        }
        ...
        //    {
        //        if (__instance.def.HasComp(typeof(CompVehicle)) && (__instance.Faction == null || !__instance.Faction.IsPlayer))
        //        {
        //            CompVehicle comp = __instance.TryGetComp<CompVehicle>();
        //            {
        ...
        private static void AllowTool_Compatibility(HarmonyInstance harmony)
        {
            harmony.Patch(AccessTools.Method("AllowTool.PartyHuntController:TryGetGizmo"), new HarmonyMethod(typeof(Harmony), nameof(GizmoCompatability_Prefix)));
        }

        private static void IgniteEverything_Compatibility(HarmonyInstance harmony)
        {
            harmony.Patch(AccessTools.Method("IgniteEverything.Harmony_PawnAttackGizmoUtility:GetAttackGizmos_Postfix"), new HarmonyMethod(typeof(Harmony), nameof(GizmoCompatability_Prefix)));
        }
